...
async def decrypt_and_merge_video(mpd_url, keys_string, output_path, output_name, quality="720"):
    try:
        ...
        # Enhanced Railway-optimized yt-dlp command based on guidance
        cmd1 = f'yt-dlp -f "bv[height<={quality}]+ba/b" -o "{output_path}/file.%(ext)s" --concurrent-fragments 16 --fragment-retries 10 --http-chunk-size 10M --buffer-size 16K --allow-unplayable-format --no-check-certificate "{mpd_url}"'
...
async def download_video(url,cmd, name):
    ...
    try:
        ...
        # CRITICAL FIX: Return None instead of invalid string when no file found
